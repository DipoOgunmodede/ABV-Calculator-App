<template>
    <label class="text-primary" :ref="`preset-label-${index}`" :for="`shot-input-${index}`">{{ presetName }}</label>
    ...
    <div class="flex justify-center"><button class="px-4 py-2 hover:border-primary hover:text-white rounded border-2 border-secondary text-primary" @click.prevent="emitSelectedPresetIndex(index)">Add {{ calculateMeasure }}</button></div>
</template>
<script>
export default {
    props: {
